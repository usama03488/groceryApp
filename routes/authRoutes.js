...
router.post("/forgetPassword", async (req, res) => {
  ...
  let mailOptions = {
    ...
    html: `<a href="http://localhost:3001/updatePassword/${token}/${email}">Click This Link to update Your Password</a>`,
  };
...
//   var newArray = [];
//   fetch("https://demo.thingsboard.io/api/v1/1tyR0ef45bXKHOGXo2mN/attributes")
//     .then((data) => {
//       for (var key in data.client) {
//         if (data.client.hasOwnProperty(key)) {
//           newArray.push(data.client[key]);
//         } else {
//           res.send({ status: "No Jar Exists" });
//         }
//       }
//       res.send({ status: newArray });
//     })
...
router.post("/adminSignup", async (req, res) => {
  ...
  try {
    ...
    let mailOptions = {
      ...
      html: `<a href="http://localhost:3001/confirmAdminSignup/${token}/${email}">Click This Link to verify your Email</a>`,
    };
